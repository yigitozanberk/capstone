...
getSQLFinalProbs = function(bigPre, unigs) {
    ...
    if(nrow(my.dat) < 1) {
        ...
        if(nrow(my.dat) < 1) {
            my.dat = getUnigProbs(unigs)[, freq := freq * 0.4]
            return(my.dat)
        } else {
            my.dat = my.dat[, freq := freq * 0.16]
            return(my.dat)}
    } else {
        total_count = my.dat[, sum(freq)]
...
getSQLTrigProbs = function(bigPre, trigrams) {
    ...
    my.dat = as.data.table(sqldf(querry))
    total_count = my.dat[, sum(freq)]
    my.dat = my.dat[ , freq := freq / total_count][1:50]
    return(my.dat)
}
